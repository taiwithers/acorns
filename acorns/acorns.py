...
CONTACT: jonathan.d.henshaw@gmail.com
"""
...
class Acorns(object):
    ...
    def process(data, cluster_criteria, method = "PP", \
        ...
        Agglomerative hierarchical clustering of data. Designed for astronomical
        position-position-velocity (PPV) data (also works with PP or PPP data).
        ...
        dataarr_acorns = np.array([dataarr[:,0],dataarr[:,1],
                                   dataarr[:,2],dataarr[:,3],dataarr[:,4]])

        pixel_size = 1.0
        min_radius = (3.3848*pixel_size)/2.
        min_height = 3.0*np.mean(dataarr_acorns[3,:])
        velo_link = 0.4
        stop = 3.
...
#==============================================================================#
    ...
    def load_from(filename):
        """
        Loads a previously computed acorns file
        """
        from .io import load
...
def gen_cluster_arr(self, data, stop):
    """
    Returns the empty acorns array
...
def find_unassigned_data(self, data, stop):
    ...
    sortidx = argsort(self.unassigned_data[2,:], reversed=True)
    self.unassigned_data = self.unassigned_data[:,sortidx]
    self.cluster_arr = self.cluster_arr[:,sortidx]
...
def get_data_index(self, data, data_point):
    ...
    Notes
    -----
    There has to be a better way to do this, but it works so whatever.
...
def get_links(self, index, current_index, tree, n_jobs, re=False):
    ...
    sep = None
    if self.method <= 1:
        # Make an initial query based on distance
        ...
        if len(self.cluster_criteria) != 1:
            for i in range(len(self.cluster_criteria)-1):
                # Out of the data returned in the initial query, make further
                # queries based on additional linking criteria.
...
def local_links(self, index, data, cluster, linked_clusters, re=False):
    ...
    Notes
    -----
    Bad links can be formed on the basis of a connection between only two data
    points. This code looks at the nearest neighbours of the cluster and tries
    to prevent spurious links.
...
def check_other_components(self, index, current_idx, data_idx, data, _linked_clusters, _cluster, tree, n_jobs, re=False):
    """
    Check to see if better links can be formed. Returns the linked cluster list.
    This is only performed during PPV clustering.

    Notes
    -----
    Just because something can be linked doesn't mean it should be. This code
    performs two main tests:

    1. We check to see if any components extracted from the same pixel as the
    current bud_cluster would be more suitably linked to the current
    linked_clusters. If so, then we will remove these clusters. This is
    basically to try and give lower intensity data points (who would forge
    stronger links) a chance in the clustering.

    2. In the case where there are two (or more) linked clusters, we want to
    make sure that it is suitable to form a branch between them. So we check the
    properties of the branches. If their characteristic properties lie further
    than 3 sigma away from one another, we don't link them.
    ...
    if np.size(idx) > 1:
        ...
        for j in range(np.size(idx)):
            ...
            if not re:
                _cluster_idx = idx[j]
                _bud_cluster = Cluster(_data_point, _data_idx, idx=_cluster_idx, acorns=self)
...
def find_linked_clusters_single_antecessor(self, data, cluster, linked_clusters):
    ...
    sort_idx = argsort(floor, reversed=True)
    ...
    for j in range(len(linked_clusters)):
        ...
        if linked_clusters[j].antecedent is None:
    ...
    if len(linked_buds) > 1:
        ...
        if np.any(do_not_merge):
            ...
            for bud_cluster in linked_buds:
                ...
                update_cluster_array(self, bud_cluster, -1)
...
def remdup_preserve_order(lst):
    """
    Removes duplicates from a list but maintains the order.
...
def multi_component_check(self, data, cluster, linked_clusters, re = False):
    ...
    # Check the bud cluster against the linked clusters
    ...
    # For multiple linked clusters, check them against themselves
    if len(linked_clusters) > 1:
...
def add_to_cluster_dictionary(self, cluster):
    """
    Add a cluster to the cluster dictionary. Update the acorns array.
...
def merge_into_cluster(self, data, linked_cluster, cluster, re = False):
    """
    Merge two clusters together. Update the acorns array.
    ...
    if re is False:
        ...
        linked_cluster = merge_clusters(linked_cluster, cluster, data)
    update_cluster_array(self, cluster, linked_cluster.cluster_idx)
...
def update_cluster_array(self, cluster, value):
    """
    Updates the cluster_arr.

    """
    for j in range(0, len(cluster.cluster_indices)):
        self.cluster_arr[1, cluster.cluster_indices[j]] = int(value)
...
def relax_method(self, step, data, method, verbose, tree, n_jobs, second_pass = False):
    ...
    linking constraints will be relaxed and acorns will make a
...
def get_forest(self, verbose):
    """
    Returns the "forest" - an accessible dictionary that can be used to plot
    a dendrogram
...
def update_clusters(self, data):
    """
    Remove any clusters with number_of_members < self.minnpix_cluster, update
    the cluster_arr, and generate new unassigned_data from the bud clusters
...
def print_to_terminal(self, step, data, count, unassigned_array_length, method, re=False, second_pass=False):
    ...
    if (re is False) and (second_pass is False):
        ...
        print('')
        print("acorns will look for clusters within {}% of the data: ".format((100 * unassigned_array_length / data[0,:].size)))
        print('')
    ...
    elif (re is True) and (second_pass is False):
        inc = self.relax/self.nsteps
